...
class TobaSubGalleryService
{
    ...
    {
        $tobaSubGalleries = TobaSubGallery::all();
        if (request()->is('api/*')) {
            return makeResponse('success', 'List', Response::HTTP_OK, new TobaSubGalleryCollection($tobaSubGalleries));
        } else {
            return view('admin.tobaSubGallery.index', compact('tobaSubGalleries'));
        }
    }
    ...
    public function create()
    {
        if (request()->is('api/*')) {
            ...
            return view('admin.tobaSubGallery.createGallery');
        }
    }
    ...
    public function store(/*TobaSubGalleryStore*/Request $request)
    {
        ...
        try {
            foreach ($request->title as $key=>$value){
                $tobaSubGallery = TobaSubGallery::create(['toba_gallery_id'=>$request->toba_gallery_id,
                    'title'=>$value,
...
//                    'compressed'=>$request->compressed,
    ...
    public function show(TobaSubGallery $tobaSubGallery)
    {
        if (request()->is('api/*')) {
            ...
            return view('admin.tobaSubGallery.show', compact('tobaSubGallery'));
        }
    }
