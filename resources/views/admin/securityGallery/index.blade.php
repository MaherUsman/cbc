...
@section('content')
    ...
    @include('layouts.admin.includes.breadcrumbs' , ['breadcrumbs' => [['name' =>  __('securityGalleries.name') , 'route' => null]],
'pageTitle' => $security->title.' '.__('securityGalleries.pageTitle')
])

    <div class="row">
        <div class="col-lg-12">
            <div class="text-end">
            <a href="{{ route('securityGalleries.create', ['security'=>$security]) }}"
               class="btn btn-primary mb-4">{{ __('securityGalleries.admin.create.create') }}</a>
            </div>
            @if($security->galleries->count() > 0)
            <div class="row tab-content" id="galleryContainer">
                @include('admin.securityGallery.gallery_items', ['securityGalleries' => $securityGalleries])
            </div>
            <!-- Load More Button -->
            @if($securityGalleries->hasMorePages())
                <div class="d-flex justify-content-center mt-3">
                    <button id="loadMore" class="btn btn-primary" data-url="{{ $securityGalleries->nextPageUrl() }}">
                        Load More
                    </button>
                </div>
            @endif
    ...
    @include('admin.securityGallery.edit_modal')
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', '.editImage', function () {
                ...
                $('#editImageForm').attr('action', '/admin/securityGalleries/' + galleryId);
                ...
                $.ajax({
                    ...
                    success: function (response, status, xhr) {
                        if (xhr.status == 204) {
                            ...
                            setTimeout(function () {
                                window.location.href = "{{route('securityGalleries.index',$security)}}";
                            }, 1000);
