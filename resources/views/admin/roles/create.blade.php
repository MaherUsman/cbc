...
@section('content')
    @include('layouts.admin.includes.breadcrumbs', ['breadcrumbs' => [['name' => __('roles.admin.breadcrumbs.name'), 'route' => 'admin.roles.index'],
       ['name' => __('roles.admin.breadcrumbs.create_role'), 'route' => null]], 'pageTitle' => 'Create Role'
    ])
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">{{ __('roles.admin.create.create') }}</h6>
                    <form method="POST" action="{{ route('admin.roles.store') }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <label class="form-label">{{ __('roles.admin.create.name') }}<span class="text-danger">*</span></label>
                                <input type="text" data-rule-required="true" data-msg-required="{{ __('roles.admin.create.name_message') }}" name="name" value="{{ old('name') }}" class="form-control" placeholder="{{ __('roles.admin.create.name') }}">
                            </div>
                        </div>
                        <br>
                        <div class="row">
                        ...
                        @php
                            $sections = [
                                ...
                                'courses' => ['permissions' => ['view_course', 'add_course', 'edit_course', 'delete_course', 'edit_course', 'delete_course']],
                            ];
                        @endphp
                        @foreach($sections as $section => $details)
                                ...
                                <div class="col-md-8 d-flex flex-wrap">
                                    @foreach($details['permissions'] as $permission)
                                        <div class="col-md-3 mb-3">
                                            <div class="checkbox">
                                                <label>
                                                    <input type="checkbox" name="permissions[]" value="{{ $section . '.' . $permission }}" class="input-icheck" {{ is_array(old('permissions')) && in_array($section . '.' . $permission, old('permissions')) ? 'checked' : '' }}> {{ __('roles.admin.create.' . $permission) }}
                                                </label>
                                            </div>
                                        </div>
                                    @endforeach
                            ...
                            <hr>
                        @endforeach
                        <button type="submit" class="btn btn-primary submit">{{ __('roles.admin.create.submit') }}</button>
                    </form>
                </div>
            </div>
        </div>
