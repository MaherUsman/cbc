...
@section('content')
    @include('layouts.admin.includes.breadcrumbs', [
        'breadcrumbs' => [['name' => __('intros.admin.breadcrumbs.name'), 'route' => 'intros.index'],
        ['name' => __('intros.admin.breadcrumbs.create'), 'route' => 'intros.create']],
        'pageTitle' => __('intros.admin.breadcrumbs.create')
    ])
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">{{__('intros.admin.create.create')}}</h6>
                    <form method="POST" id="formValidation" action="{{route('intros.store')}}"
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="mb-3">
                                    <label class="form-label">{{__('intros.admin.create.title')}}<span
                                            class="text-danger">*</span> </label>
                                    <input type="text" data-rule-required="true"
                                           data-msg-required="{{__('intros.admin.create.title_message')}}"
                                           name="title" value="{{old('title')}}" class="form-control"
                                           placeholder="{{__('intros.admin.create.title')}}">
                                </div>
                            </div>
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label class="form-label"
                                    >{{__('intros.admin.create.details')}}</label>
                                    <textarea name="details" id="ckeditor"></textarea>
                                </div>
                            </div>
                            <div class="col-sm-12">
                                <div class="mb-3">
                                    <label class="form-label">{{__('intros.admin.create.image')}}<span
                                            class="text-danger">*</span></label>
                                    <input type="file" name="image" class="form-control" id="imageUpload"
                                           accept="image/*" data-rule-required="true"
                                           data-msg-required="{{__('intros.admin.create.slink_message')}}">
                                </div>
                                <div class="mb-3">
