...
@section('content')
    ...
    @include('layouts.admin.includes.breadcrumbs' , ['breadcrumbs' => [['name' =>  __('tobaGallery.name') , 'route' => null]],
'pageTitle' => __('tobaGallery.pageTitle')
])
...
{{--                            <h4 class="card-title">{{ __('tobaGallery.list_tobaGallery') }}</h4>--}}
                            <button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target=".bd-example-modal-lg">{{ __('tobaGallery.add_tobaGallery') }}</button>
                                        ...
                                        <div class="gallery-overlay rounded">
                                            <div class="overlay-icons-wrapper w-100 d-flex flex-column align-items-end">
                                                <div class="overlay-icon">
                                                    <a href="{{route('toba-sub-galleries.index', $gallery->id)}}" title="Add Child Gallery Images" data-bs-toggle="tooltip" data-bs-placement="top">
                                                        <i class="fa-solid fa-plus"></i>
                                                    </a>
                                                </div>
                                                <div class="overlay-icon mt-2">
                                                    <a href="{{route('toba-galleries.edit', $gallery)}}" title="Edit Gallery Image" data-bs-toggle="tooltip" data-bs-placement="top">
                                                        <i class="fa-solid fa-pen-to-square"></i>
                                                    </a>
                                                </div>
                                                <div class="overlay-icon mt-2">
                                                    <a href="#" data-url="{{ route('toba-galleries.destroy', $gallery) }}" title="Delete"
    ...
    <script>
        $(document).ready(function () {
            $('.bd-example-modal-lg').on('shown.bs.modal', function () {
                if (tinymce.get('modalDescription')) {
                    tinymce.get('modalDescription').remove();
                }
                tinymce.init({
            ...
            $('#formValidation').validate({
                submitHandler: async function (form, event) {
                    ...
                    var descriptionInputs = $('textarea[name="description[]"]');
                    ...
                    descriptionInputs.each(function (index, element) {
                        data.append('description[]', $(element).val());
                    });
