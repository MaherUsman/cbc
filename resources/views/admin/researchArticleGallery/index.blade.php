...
@section('content')
    ...
    @include('layouts.admin.includes.breadcrumbs' , ['breadcrumbs' => [['name' =>  __('researchArticleGalleries.name') , 'route' => null]],
'pageTitle' => $researchArticle->title.' '.__('researchArticleGalleries.pageTitle')
])

    <div class="row">
        <div class="col-lg-12">
            <div class="text-end">
            <a href="{{ route('researchArticleGalleries.create', $researchArticle->id) }}"
               class="btn btn-primary mb-4">{{ __('researchArticleGalleries.admin.create.create') }}</a>
            </div>
            @if($researchArticle->galleries->count() > 0)
            <div class="row tab-content" id="galleryContainer">
                @include('admin.researchArticleGallery.gallery_items', ['researchArticleGalleries' => $researchArticleGalleries])
            </div>
            <!-- Load More Button -->
            @if($researchArticleGalleries->hasMorePages())
                <div class="d-flex justify-content-center mt-3">
                    <button id="loadMore" class="btn btn-primary" data-url="{{ $researchArticleGalleries->nextPageUrl() }}">
                        Load More
                    </button>
                </div>
            @endif
    ...
    @include('admin.researchArticleGallery.edit_modal')
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', '.editImage', function () {
                ...
                $('#editImageForm').attr('action', '/admin/researchArticleGalleries/' + galleryId);
                ...
                $.ajax({
                    ...
                    success: function (response, status, xhr) {
                        if (xhr.status == 204) {
                            ...
                            setTimeout(function () {
                                window.location.href = "{{route('researchArticleGalleries.index',$researchArticle)}}";
                            }, 1000);
