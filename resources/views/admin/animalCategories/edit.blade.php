...
@section('content')
    @include('layouts.admin.includes.breadcrumbs', [
        ...
        ['name' => __('animalCategories.admin.breadcrumbs.edit'), 'route' => null]],
        'pageTitle' => __('animalCategories.admin.breadcrumbs.edit')
    ])
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
{{--                    <h6 class="card-title">{{__('animalCategories.admin.edit.edit')}}</h6>--}}
                    <form method="POST" id="formValidation" action="{{route('animal-categories.update',$animalCategory)}}"
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">{{__('animalCategories.admin.edit.name')}}<span
                                            class="text-danger">*</span> </label>
                                    <input type="text" data-rule-required="true"
                                           data-msg-required="{{__('animalCategories.admin.edit.name_message')}}"
                                           name="name" value="{{$animalCategory->name}}" class="form-control"
                                           placeholder="{{__('animalCategories.admin.edit.name')}}">
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">{{__('animalCategories.admin.create.slug')}}<span
                                            class="text-danger"></span> </label>
                                    <input type="text" data-msg-required="{{__('animalCategories.admin.edit.title_message')}}"
                                           name="slug" value="{{ $animalCategory->slug }}" class="form-control"
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description">{!! $animalCategory->description !!}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <label class="form-label">{{__('animalCategories.admin.edit.image')}}</label>
                                    <input type="file" name="image" class="form-control" id="imageUpload"
                                           accept="image/*">
                                </div>
                                <div class="mb-3">
                                    <img id="imagePreview" src="{{asset($animalCategory->image?:'no_image.jpg')}}"
                                         alt="Image Preview" class="img-thumbnail"
                                         style="{{$animalCategory->image?'':'display:none;'}} max-width:200px; height:auto;">
                                </div>
                            </div><!-- Col -->
                        </div><!-- Row -->
                        ...
                        <button type="submit" class="btn btn-primary submit">
                            {{__('animalCategories.admin.edit.submit')}}
                        </button>
                    </form>
                </div>
            </div>
