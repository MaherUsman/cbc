...
@section('content')
    ...
    @include('layouts.admin.includes.breadcrumbs' , ['breadcrumbs' => [['name' =>  __('blogGalleries.name') , 'route' => null]],
'pageTitle' => $blog->title.' '.__('blogGalleries.pageTitle')
])

    <div class="row">
        <div class="col-lg-12">
            <div class="text-end">
            <a href="{{ route('blog-galleries.create', $blog->id) }}"
               class="btn btn-primary mb-4">{{ __('blogGalleries.admin.create.create') }}</a>
            </div>
            <div class="row tab-content" id="galleryContainer">
                @include('admin.blogGallery.gallery_items', ['blogGalleries' => $blogGalleries])
            </div>
            <!-- Load More Button -->
            @if($blogGalleries->hasMorePages())
                <div class="d-flex justify-content-center mt-3">
                    <button id="loadMore" class="btn btn-primary" data-url="{{ $blogGalleries->nextPageUrl() }}">
                        Load More
                    </button>
                </div>
            @endif
    ...
    @include('admin.blogGallery.edit_modal')
    ...
    <script>
        $(document).ready(function () {
            $(document).on('click', '.editImage', function () {
                ...
                $('#editImageForm').attr('action', '/admin/blog-galleries/' + galleryId);
