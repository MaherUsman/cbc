...
@section('content')
    @include('layouts.admin.includes.breadcrumbs', [
        'breadcrumbs' => [
            ...
            ['name' => __('blogGalleries.admin.breadcrumbs.edit'), 'route' => 'blog-galleries.edit', 'params' => $blogGallery->blog]
        ],
        'pageTitle' => __('blogGalleries.admin.breadcrumbs.edit')
    ])
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">{{__('blogGalleries.admin.edit.edit')}}</h6>
                    <form method="POST" id="formValidation" action="{{route('blog-galleries.update',$blogGallery)}}"
                        ...
                        <div class="row rowTemplate">
                            <div class="col-sm-5">
                                <div class="mb-3">
                                    <label class="form-label">{{__('blogGalleries.admin.create.title')}}{{--<span
                                            class="text-danger">*</span> --}}</label>
                                    <input type="text" data-rule-required="false"
                                           data-msg-required="{{__('blogGalleries.admin.create.title_message')}}"
                                           name="title" class="form-control" value="{{$blogGallery->title}}"
                                           placeholder="{{__('blogGalleries.admin.create.title')}}">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="mb-3">
                                    <label class="form-label">{{__('blogGalleries.admin.create.image')}}<span
                                            class="text-danger">*</span></label>
                                    <input type="file" name="image" id="imageUpload" class="form-control" accept="image/*"
                                           ...
                                           data-msg-required="{{__('blogGalleries.admin.create.image_message')}}">
                                </div>
                            </div>
                            <div class="col-sm-3">
                                <div class="mb-3">
                                    <img src="{{asset($blogGallery->image)}}" alt="Image Preview" class="img-thumbnail"
                                         style="display:block; max-width:200px; height:auto;">
                                </div>
                            </div>
                        </div>

                        <a href="{{route('blog-galleries.index', $blogGallery->blog)}}" class="btn btn-danger light btn-sl-sm" type="button">
                            {{__('blogGalleries.admin.form.cancel')}}
                        </a>
                        <button type="submit" class="btn btn-primary submit">
                            {{__('blogGalleries.admin.create.submit')}}
                        </button>
                    </form>
        ...
        $(document).ready(function () {
            ...
            async function submitFormData(url, data) {
                ...
                try {
                    ...
                    setTimeout(function () {
                        window.location.href = "{{route('blog-galleries.index', $blogGallery->blog)}}";
                    }, 1000);
                } catch (xhr) {
                    $.unblockUI();
